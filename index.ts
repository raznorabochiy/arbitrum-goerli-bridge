...
async function bridge(key: string): Promise<boolean> {
  const provider = providers.get(Network.GOERLI);
  ...
  try {
    ...
    cli.spinner(getTxLink(Network.GOERLI, tx.hash), true);
...
for (let i = 0; i < keys.length; i++) {
  ...
  try {
    const goerliEthBalance = await getBalance(
      Network.ARBITRUM_GOERLI,
      address,
    );

    if (goerliEthBalance < parseEther(MAX_BRIDGE_IN_ETH.toString())) {
      console.log(
        `ETH в сети ${Network.GOERLI} меньше чем ${MAX_BRIDGE_IN_ETH}`,
      );
      continue;
    }
    ...
    if (arbGoerliEthBalance === 0n) {
      console.log(`Нет ETH в сети ${Network.ARBITRUM_GOERLI}`);
      console.log(`Делаем бридж из ${Network.GOERLI}`);
      ...
      console.log(
        `В сети ${Network.ARBITRUM_GOERLI} уже есть ${
